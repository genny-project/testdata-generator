    ...
    public static class GeneratorTask implements Runnable {
        ...
        private void generateOne() {
            try {
                ...
                // DEF_EDU_PROVIDER
                BaseEntity eduProvider = generator.generateEntity(Entities.DEF_EDU_PROVIDER);
                // DEF_EDU_PRO_REP
                List<BaseEntity> eduProviderReps = new ArrayList<>(maxReps);
                for (int i = 0; i < maxReps; i++)
                    eduProviderReps.add(generator.generateEntity(Entities.DEF_EDU_PRO_REP));
                // DEF_APPLICATION
                ...
                generator.createRelation(hostCompany, eduProvider,
                        SpecialAttributes.LNK_EDU_PROVIDER, null);
                // DEF_EDU_PROVIDER <--> DEF_EDU_PRO_REP
                generator.createRelation(eduProvider, eduProviderReps, null,
                        SpecialAttributes.LNK_EDU_PROVIDER);
                // DEF_EDU_PROVIDER <--> DEF_INTERN
                generator.createRelation(eduProvider, interns, null,
                        SpecialAttributes.LNK_EDU_PROVIDER);
                for (int i = 0; i < fixedSize; i++) {
                    ...
                    generator.createRelation(applications.get(i), eduProvider,
                            SpecialAttributes.LNK_EDU_PROVIDER, null);
                }
            } catch (Throwable e) {
                listener.onError(e);
